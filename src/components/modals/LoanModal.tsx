...
export const LoanModal: React.FC<LoanModalProps> = ({ onClose }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.receiveDate.trim()) {
      newErrors.receiveDate = 'کادر الزامی است';
    }
  ...
  const handleSave = () => {
    if (validateForm()) {
      console.log('Saving loan data:', formData);
      onClose();
    }
  };
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md rtl">
        <DialogHeader className="flex flex-row items-center justify-between">
          <DialogTitle>فرم وام</DialogTitle>
          <Button
            variant="ghost"
            size="sm"
            onClick={onClose}
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="receiveDate" className="text-right flex justify-end">
              تاریخ دریافت وام <span className="text-destructive mr-1">*</span>
            </Label>
            <div className="relative">
              <Input
                id="receiveDate"
            ...
            {errors.receiveDate && (
              <p className="text-sm text-destructive text-right">{errors.receiveDate}</p>
            )}
          </div>
        </div>
